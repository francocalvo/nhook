...
class TestPasajesSyncWorkflow:
    def test_matches_with_correct_workflow_name(self, mock_notion_client: AsyncMock) -> None:
    ...
    def test_matches_with_lowercase_departure(self, mock_notion_client: AsyncMock) -> None:
        workflow = PasajesSyncWorkflow(mock_notion_client)
        context = WorkflowContext(
            page_id="test-id",
            payload={"data": {"properties": {"departure": {"start": "2026-03-14"}}},
    ...
    def test_does_not_match_wrong_workflow_name(self, mock_notion_client: AsyncMock) -> None:
        workflow = PasajesSyncWorkflow(mock_notion_client)
        context = WorkflowContext(
            page_id="test-id",
    ...
    @pytest.mark.asyncio
    async def test_execute_clears_relation_when_departure_is_none(self, mock_notion_client: AsyncMock) -> None:
        workflow = PasajesSyncWorkflow(mock_notion_client)
        context = WorkflowContext(
    ...
    @pytest.mark.asyncio
    async def test_execute_finds_and_links_cronograma_entries(self, mock_notion_client: AsyncMock) -> None:
